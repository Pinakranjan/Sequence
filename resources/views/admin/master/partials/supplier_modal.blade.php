...
{{-- Reusable Supplier Add/Edit/View Modal --}}
<div class="modal fade" id="{{ $buildId('supplierModal') }}" tabindex="-1"
    aria-labelledby="{{ $buildId('supplierModalLabel') }}" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:760px;">
        <div class="modal-content">
            <div class="modal-header py-2" style="min-height:42px;">
                <h5 class="modal-title d-flex align-items-center fs-6" id="{{ $buildId('supplierModalLabel') }}">
                    <i class="fa-solid fa-truck-field me-2" style="width:18px;height:18px;line-height:18px;"></i>
                    <span id="{{ $buildId('modalTitle') }}">Add Supplier</span>
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form id="{{ $buildId('supplierForm') }}" method="POST" action="{{ url('/supplier/create') }}"
                ...
                <input type="hidden" id="{{ $buildId('supplier_id') }}" name="id" />
                <div class="modal-body">
                    <div class="col-12">
                        <div class="row g-2 align-items-start">
                            <div class="col-12 col-md-8 pe-md-2">
                                <label for="{{ $buildId('supplier_name') }}" class="form-label">Supplier Name <span
                                        class="text-danger">*</span></label>
                                <input type="text" id="{{ $buildId('supplier_name') }}" name="supplier_name"
                                    class="form-control" maxlength="100" autocomplete="off" required />
                                <div class="invalid-feedback" id="{{ $buildId('supplier_name_error') }}">Supplier name
                                    is required.</div>
                            </div>
                            <div class="col-12 col-md-4 ps-md-2">
                                <label for="{{ $buildId('supplier_code') }}" class="form-label">Supplier Code <span
                                        class="text-danger">*</span></label>
                                <input type="text" id="{{ $buildId('supplier_code') }}" name="supplier_code"
                                ...
                                <div class="invalid-feedback" id="{{ $buildId('supplier_code_error') }}">Supplier code
                                    is required.</div>
                            </div>
                        </div>
                    </div>
