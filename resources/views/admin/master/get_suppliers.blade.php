...
@php
	...
	$pageTitle = $pageTitle ?? 'Supplier Master';
	...
	if (!$isAdminRole) {
		$row = DB::table('utility_user_permission_register')->where('user_id', $auth->id)->where('form_id', 2)->select('is_add','is_edit','is_delete')->first();
...
@include('components.common_js', [
	...
	'gridId' => 'supplierGrid', 'bodyHeight' => '500px',
	'hideAddButton' => $hideAddButton, 'showUserAvatar' => false,
	'emptyEntity' => 'Supplier Register'
])
...
(function(){
	...
	document.addEventListener('DOMContentLoaded', function(){
		...
		const listUrl = @json(route('list.suppliers'));
		const addUrl = @json(route('add.supplier'));
		const updateUrl = @json(route('update.supplier'));
		const removeUrl = @json(route('delete.supplier'));
		const setActiveUrl = @json(route('setactive.supplier'));
		...
		const NO_BUSINESS_TEMPLATE = '<div style="text-align:center;padding:40px 20px;"><div style="font-size:42px;color:#94a3b8;margin-bottom:10px;">🏢</div><h5 style="color:#64748b;margin-bottom:6px;">No Business Selected</h5><p style="color:#94a3b8;margin:0;">Select a business from the top-right to view Suppliers</p></div>';

		const gridOptions = Object.assign({}, AppUtils.GridHelpers.baseGridOptions(), {
			emptyRecordTemplate: IS_SUPER ? NO_BUSINESS_TEMPLATE : AppUtils.emptyRecordTemplate('Supplier Register'),
			columns: [
				...
				{ field: 'supplier_name', headerText: 'Supplier Name', width: 240, validationRules: { required: true }, template: (args) => {
					const name = AppUtils.escapeHtml(String(args.supplier_name || ''));
					var inactivePill = '';
					...
					var code = (args && args.supplier_code) ? AppUtils.escapeHtml(String(args.supplier_code)) : '';
					var codeBadge = '';
					if (code) {
				...
				{ field: 'supplier_code', headerText: 'Code', width: 110, visible: false },
			...
			pdfQueryCellInfo: (function(){
				...
				return function(args){ try { base(args); } catch(_){ } try { if (args && args.column && args.data) { if (args.column.field === 'supplier_name') { let txt = String(args.data.supplier_name||''); if (!isActiveVal(args.data.is_active)) txt += ' [Inactive]'; const code = String(args.data.supplier_code||''); if (code) txt += ' ('+code+')'; args.value = txt; } if (args.column.field === 'is_active_text') args.value = String(args.data.is_active_text||''); } } catch(_){ } };
			})(),
		...
		const grid = new ej.grids.Grid(Object.assign({}, gridOptions, {
			commandClick: function(args){
				try {
					...
					if (target.closest && target.closest('.cmd-delete')) { if (!PERMS.delete) return; AppUtils.Actions.deleteResource({ url: removeUrl, method: 'POST', data: { id: row.id }, confirm: { title:'Are you sure?', text:'Delete this supplier?' }, onSuccess: refreshGrid }); return; }
		...
		const inputId = document.getElementById('supplier_id');
		...
		function populateForm(row, mode){
			...
			if (inputId) inputId.value = row.id || ''; if (codeEl) codeEl.value = row.supplier_code || ''; if (nameEl) nameEl.value = row.supplier_name || '';
		...
		function onSetActive(val, id){
			...
			AppUtils.Actions.deleteResource({ url: setActiveUrl, method: 'POST', data: { id: id, is_active: val }, confirm: { title: 'Are you sure?', text: val ? 'Activate this supplier?' : 'Deactivate this supplier?', confirmButtonText: val ? 'Yes, activate' : 'Yes, mark inactive' }, onSuccess: refreshGrid });
		}
		...
		if (window.AppUtils && AppUtils.FormHelpers && form) {
			AppUtils.FormHelpers.attachAjaxSubmit(form, {
				...
				errorTargets: { supplier_code: '#supplier_code_error', supplier_name: '#supplier_name_error', email: '#email_error', mobile_no: '#mobile_no_error', gstin: '#gstin_error', pan: '#pan_error', correspondence_address: '#correspondence_address_error' },
				beforeSubmit: ({ form, mode }) => { if (mode !== 'edit') { try { form.querySelector('#supplier_id').value = ''; } catch(_) {} } if (IS_SUPER) { const cid = getSelectedBusinessId(); if (!cid) { AppUtils.notify('Please select a business first.', { type: 'error' }); return false; } if (formCompanyIdEl) formCompanyIdEl.value = cid; } },
				onSuccess: function(json, { mode }){ AppUtils.notify((json && json.message) || (mode === 'edit' ? 'Supplier updated' : 'Supplier created'), { type: (json && json['alert-type']) || 'success' }); try { closeModal(); } catch(_) {} refreshGrid(); }
			});
		}
