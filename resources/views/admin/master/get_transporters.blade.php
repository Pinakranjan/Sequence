...
@php
	...
	$pageTitle = $pageTitle ?? 'Transporter Master';
	$pageIcon = $pageIcon ?? 'truck';
	...
	if (!$isAdminRole) {
		$row = DB::table('utility_user_permission_register')->where('user_id', $auth->id)->where('form_id', 3)->select('is_add','is_edit','is_delete')->first();
...
@include('components.common_js', [
	...
	'gridId' => 'transporterGrid', 'bodyHeight' => '500px',
	'hideAddButton' => $hideAddButton, 'showUserAvatar' => false,
	'emptyEntity' => 'Transporter Register'
])
...
(function(){
	...
	document.addEventListener('DOMContentLoaded', function(){
		...
		const listUrl = @json(route('list.transporters'));
		const addUrl = @json(route('add.transporter'));
		const updateUrl = @json(route('update.transporter'));
		const removeUrl = @json(route('delete.transporter'));
		const setActiveUrl = @json(route('setactive.transporter'));
		...
		const NO_BUSINESS_TEMPLATE = '<div style="text-align:center;padding:40px 20px;"><div style="font-size:42px;color:#94a3b8;margin-bottom:10px;">🏢</div><h5 style="color:#64748b;margin-bottom:6px;">No Business Selected</h5><p style="color:#94a3b8;margin:0;">Select a business from the top-right to view Transporters</p></div>';

		const gridOptions = Object.assign({}, AppUtils.GridHelpers.baseGridOptions(), {
			emptyRecordTemplate: IS_SUPER ? NO_BUSINESS_TEMPLATE : AppUtils.emptyRecordTemplate('Transporter Register'),
			columns: [
				...
				{ field: 'transporter_name', headerText: 'Transporter Name', width: 240, validationRules: { required: true }, template: (args) => {
					const name = AppUtils.escapeHtml(String(args.transporter_name || ''));
					var inactivePill = '';
					...
					var code = (args && args.transporter_code) ? AppUtils.escapeHtml(String(args.transporter_code)) : '';
					var codeBadge = '';
					if (code) {
				...
				{ field: 'transporter_code', headerText: 'Code', width: 110, visible: false },
				...
				{ field: 'updated_at', headerText: 'Updated At', width: 200, type: 'datetime', visible: false, format: { type: 'datetime', format: 'dd-MMM-yyyy hh:mm a' }, showInColumnChooser: showColumnChooser },
			...
			pdfQueryCellInfo: (function(){
				...
				return function(args){ try { base(args); } catch(_){ } try { if (args && args.column && args.data) { if (args.column.field === 'transporter_name') { let txt = String(args.data.transporter_name||''); if (!isActiveVal(args.data.is_active)) txt += ' [Inactive]'; const code = String(args.data.transporter_code||''); if (code) txt += ' ('+code+')'; args.value = txt; } if (args.column.field === 'is_active_text') args.value = String(args.data.is_active_text||''); } } catch(_){ } };
			})(),
		...
		const grid = new ej.grids.Grid(Object.assign({}, gridOptions, {
			commandClick: function(args){
				try {
					...
					if (target.closest && target.closest('.cmd-delete')) { if (!PERMS.delete) return; AppUtils.Actions.deleteResource({ url: removeUrl, method: 'POST', data: { id: row.id }, confirm: { title:'Are you sure?', text:'Delete this transporter?' }, onSuccess: refreshGrid }); return; }
		...
		const inputId = document.getElementById('transporter_id');
		const codeEl = document.getElementById('transporter_code');
		const nameEl = document.getElementById('transporter_name');
		...
		function generateCodeFromName(name){
			let s = String(name||'').toUpperCase().replace(/[^A-Z]/g,''); if (!s) return '';
			const vowels = new Set(['A','E','I','O','U']); let base = s[0] || 'X';
			...
			while (base.length < 3) base += 'X'; return (base + '01').slice(0,5);
		}
		...
		function applyModeUI(mode){ const ro = (mode === 'view'); ALL_INPUTS.forEach(el => { if (el) el.disabled = ro; }); if (submitBtn) submitBtn.style.display = (mode === 'view') ? 'none' : ''; if (auditCtrl) auditCtrl.showFor(mode === 'create' ? 'create' : mode); }

		function resetFormForCreate(){
			setMode('create'); setTitleFor('create'); if (form) form.reset(); if (inputId) inputId.value = ''; if (form) form.action = addUrl; applyModeUI('create');
			if (auditCtrl) auditCtrl.set({ createdAt: null, updatedAt: null }); codeTouched = false;
		...
		function populateForm(row, mode){
			setMode(mode); setTitleFor(mode); if (form) form.action = (mode === 'create') ? addUrl : updateUrl;
			if (inputId) inputId.value = row.id || ''; if (codeEl) codeEl.value = row.transporter_code || ''; if (nameEl) nameEl.value = row.transporter_name || '';
			if (contactEl) contactEl.value = row.contact_person || ''; if (emailEl) emailEl.value = row.email || ''; if (mobileEl) mobileEl.value = row.mobile_no || '';
			if (gstinEl) gstinEl.value = row.gstin || ''; if (panEl) panEl.value = row.pan || ''; if (corrAddrEl) corrAddrEl.value = row.correspondence_address || '';
			if (billAddrEl) billAddrEl.value = row.billing_address || ''; if (notesEl) notesEl.value = row.notes || '';
			applyModeUI(mode);
		...
		function onSetActive(val, id){
			...
			AppUtils.Actions.deleteResource({ url: setActiveUrl, method: 'POST', data: { id: id, is_active: val }, confirm: { title: 'Are you sure?', text: val ? 'Activate this transporter?' : 'Deactivate this transporter?', confirmButtonText: val ? 'Yes, activate' : 'Yes, mark inactive' }, onSuccess: refreshGrid });
		}

		if (window.AppUtils && AppUtils.FormHelpers && form) {
			AppUtils.FormHelpers.attachAjaxSubmit(form, {
				submitBtn, method: 'POST', getMode: () => (form.dataset.mode || 'create'),
				errorTargets: { transporter_code: '#transporter_code_error', transporter_name: '#transporter_name_error', email: '#email_error', mobile_no: '#mobile_no_error', gstin: '#gstin_error', pan: '#pan_error', correspondence_address: '#correspondence_address_error' },
				beforeSubmit: ({ form, mode }) => { if (mode !== 'edit') { try { form.querySelector('#transporter_id').value = ''; } catch(_) {} } if (IS_SUPER) { const cid = getSelectedBusinessId(); if (!cid) { AppUtils.notify('Please select a business first.', { type: 'error' }); return false; } if (formCompanyIdEl) formCompanyIdEl.value = cid; } },
				onSuccess: function(json, { mode }){ AppUtils.notify((json && json.message) || (mode === 'edit' ? 'Transporter updated' : 'Transporter created'), { type: (json && json['alert-type']) || 'success' }); try { closeModal(); } catch(_) {} refreshGrid(); }
			});
		}
		...
		if (IS_SUPER && window.AppUtils && AppUtils.BusinessPicker) {
			...
			AppUtils.BusinessPicker.init({ headerSelector: '.card-header.d-flex', noImageUrl: NO_IMAGE, listUrl: BUSINESS_PICKER_LIST_URL, addButtonHandler, onClear: function(){ grid.emptyRecordTemplate = NO_BUSINESS_TEMPLATE; grid.dataSource = []; }, onBusinessChanged: function(company){ grid.emptyRecordTemplate = AppUtils.emptyRecordTemplate('Transporter Register'); AppUtils.GridHelpers.loadDataToGrid(grid, listUrl + '?company_id=' + encodeURIComponent(String(company.id)), { dateFields: DATE_FIELDS }); } });
		}
	});
})();
</script>

@include('admin.master.partials.transporter_modal', ['idPrefix' => ''])
